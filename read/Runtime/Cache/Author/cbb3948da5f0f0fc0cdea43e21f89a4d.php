...
<script type="text/javascript">
$(function(){
    $(".main .menu ul:eq(2)").addClass("selected")
})
</script>
<style type="text/css">
.main {
...
<div class="main">
    ...
    <form method="post" action="__URL__/add" >
     <p style="height:30px;margin-top:10px;font-size:14px;">
    作品：<select name="booksId">
    	<?php if(is_array($books)): $i = 0; $__LIST__ = $books;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["bookTitle"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
    	</select>
       章节：<input name="contentNum" type="text" value="<?php echo ($bookNewNum); ?>"  />
     
        标题：<input name="contentName" type="text" />
    </p>

    <textarea id="content_1" name="contents" style="width:727px;height:350px;"></textarea> 
