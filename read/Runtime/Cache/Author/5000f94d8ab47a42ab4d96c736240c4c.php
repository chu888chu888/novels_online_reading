...
<div class="main">
   ...
   <?php if(is_array($bookcontents)): $i = 0; $__LIST__ = $bookcontents;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><form method="post" action="__URL__/save?id=<?php echo ($vo["id"]); ?>" >
     
     <p style="height:30px;margin-top:10px;font-size:14px;">
    
       章节号：<input name="contentNum" type="text" value="<?php echo ($vo["contentNum"]); ?>"  />
     
        章节标题：<input name="contentName" type="text" value="<?php echo ($vo["contentName"]); ?>"  />
    </p>

    <textarea id="content_1" name="contents" style="width:727px;height:350px;"><?php echo ($vo["contents"]); ?></textarea> 
